...
function CustomDescription({ children }) {
    ...
    const hasShortDescription =
        productContextValue?.product?.properties.find((e) => e.name == "Descrição Objetiva")?.values.length >= 1;
    const shortDescription = productContextValue?.product?.properties.find((e) => e.name == "Descrição Objetiva")
        ?.values[0];
